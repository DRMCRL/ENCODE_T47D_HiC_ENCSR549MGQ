...
```{r featureTotals}
featureTotals <- table(h3k27ac$class) %>%
...
h3k27ByGroup <- list(
  ...
  lapply(`seqinfo<-`, value = sq)
```

```{r}
h3k27ac$E2_DHT <- h3k27ac$peak_id %in% h3k27ByGroup$E2_DHT$peak_id
...
gen <- "hg19"
ax <- GenomeAxisTrack()
plotRegion <- function(gr, zoom = 1, hic = gi, h3k27 = h3k27ac, gata3 = gata3ConsGR, gm = geneModels, .ax = ax, rna = rnaseq, shift = 0){
  ...
  expTR <- subsetByOverlaps(gm, gr, ignore.strand = TRUE) %>%
  ...
  notExpTR <- subsetByOverlaps(gm, gr, ignore.strand = TRUE) %>%
    subset(!gene %in% rna$gene_id) %>%
    GeneRegionTrack(
      name = "Not Expressed",
      transcriptAnnotation = "symbol",
...
```{r gata3ConsGR}
gata3GR <- here::here("data/external/BED") %>%
...
A total of `r comma(length(gata3Cons2Feature))` peaks were mapped to `r comma(sum(vapply(gata3Cons2Feature, length, integer(1))))` features.
...
Whilst an interaction between the ZBTB16 promoter and a downstream region was detected, this region did not interact with any other GATA peaks.

```{r plotZBTB16, fig.cap = "*Overlay of HiC Interactions, GATA3 binding sites and H3K27Ac marks. For GATA3 peaks, Promoters, Enhancers and Super Ehancers, red indicates a gain under DHT, grey represents common sites across treatments and blue indicates a loss under DHT. Expressed genes are displayed in green with all transcripts showing. Undetectable genes from the RNA Seq dataset are condensed into a single representative transcript in the bottom panel.*"}
gr <- chipGR %>%
  join_overlap_intersect(h3k27ac) %>%
  subset(class == "Promoter") %>%
  join_overlap_intersect(geneRanges) %>%
...
# We should also add & colour the AR peaks by their DHT response
```
