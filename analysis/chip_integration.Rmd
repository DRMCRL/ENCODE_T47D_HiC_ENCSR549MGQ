...
```{r chipBed}
chipBed <- here::here("data/external/BED") %>%
  list.files(full.names = TRUE, pattern = "bed")
```
...
```{r plotWidths, fig.cap="*Assessment of peak width for each set of consensus peaks. H3K27Ac peaks were noticeably wider than GATA3 or AR peaks, with AR peaks also appearing wider in the presence of DHT*"}
allWidths %>%
...
allWidths %>% 
  ...
  pander(
    justify = "lllrrr",
    caption = glue(
      "Summary of ChIP-Seq peaks for each sample after restricting to a minimum peak size >{minWidth}bp."
    )
  )
```
...
Both GATA3 and AR ChIP datasets were loaded and consensus peaks identified.
...
gata3Cons2Feature <- gata3ConsGR %>% 
  ...
  mcols() %>% 
  split(f = .$peak_id.x) %>% 
  lapply(function(x){unique(x$class)})
```
...
In order to characterise the behaviours of the defined groups, the consensus set of `r comma(length(arConsGR))` AR peaks was defined as those detected in either Vehicle or DHT.
...
```{r goseqGata3}
mapped <- (gata3ConsGR$peak_id %in% chipGR$peak_id) %>%
...
rnaseq %>%
  ...
  fisher.test() %>%
  pander(
    caption = "Fisher's Exact Test providing weak support for enrichment of GATA3 peaks within the promoters of DE genes, using an FDR of 0.05 to consider a gene as DE"
  )
```
...
plotRegion(gr, zoom = 300, gm = transModels, shift = -5e3, fs = 10)
```
...
There was a discrepancy with the peak assignment to group, with four peaks being considered as DHT only, but some were found in both.
These peaks may be misclassified.
...
gr <- chipGR %>%
  ...
  # .[1] %>%
  # range()
plotRegion(gr[1], zoom = 100, gm = transModels, shift = 5e3)
plotRegion(gr[2], zoom = 100, gm = transModels)
plotRegion(gr[3], zoom = 150, gm = transModels)
plotRegion(gr[4], zoom = 300, gm = transModels)
```
...
gr <- chipGR %>%
  ...
  join_overlap_intersect(geneRanges) %>%
  subset(symbol == "GJA1")
plotRegion(gr, zoom = 50, gm = transModels)
```

#### KDM4B

These peaks again appear misclassified.
However, there appears to be be increased activity at the promoter.
The HiC interaction connects to a region with no defined genes, promoters or enhancers.
...
gr <- chipGR %>%
  ...
  join_overlap_intersect(geneRanges) %>%
  subset(symbol == "KDM4B")
plotRegion(gr[1], zoom = 800, gm = transModels, shift = 9e4)
plotRegion(gr[2], zoom = 800, gm = transModels, shift = 8e4)
```
...
gr <- chipGR %>%
  ...
  join_overlap_intersect(geneRanges) %>%
  subset(symbol == "SASH1")
plotRegion(gr[1], zoom = 1600, gm = transModels, shift = 9e4)
```
...
gr <- chipGR %>%
  ...
  join_overlap_intersect(geneRanges) %>%
  subset(symbol == "GREB1")
plotRegion(gr[1], zoom = 1000, gm = transModels, shift = 9e4)
plotRegion(gr[2], zoom = 1000, gm = transModels, shift = 9e4)
plotRegion(gr[3], zoom = 500, gm = transModels, shift = 7e4)
plotRegion(gr[4], zoom = 1000, gm = transModels, shift = 6e4)
plotRegion(gr[5], zoom = 300, gm = transModels, shift = 3e4)
```
