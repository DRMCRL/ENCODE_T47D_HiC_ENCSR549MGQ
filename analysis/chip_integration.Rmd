...
```{r arConsGR}
arGR <- here::here("data/external/BED") %>%
...
plotRegion(gr, zoom = 300, gm = transModels, shift = -5e3)
```
...
plotRegion(gr, zoom = 1000, gm = transModels)
```
...
```{r plotEHF, fig.cap = "*Zoomed view of EHF showing the two peaks assigned as AR-independent DHT-responsive. The first was clearly detected in GATA3 (Vehicle) as well as GATA3 + DHT, whilst the second one was corectly annotated.*"}
gr <- chipGR %>%
  join_overlap_intersect(h3k27ac) %>%
  subset(class == "Promoter") %>%
  join_overlap_intersect(geneRanges) %>%
  ...
  .[1:2] %>%
  range()
plotRegion(gr, zoom = 20, gm = transModels)
```

```{r plotEHFExpanded, fig.cap = "*Expanded view of the above showing HiC contact locations*"}
plotRegion(gr, zoom = 420, gm = transModels, shift = 8e4)
```
