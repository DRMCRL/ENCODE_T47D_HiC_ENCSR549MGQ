...
Using `r sz/1e3`kb bins, a total of `r nrow(cis_int)` non-zero interactions were observed between bin pairs.
...
```{r, fig.height=6, fig.cap = glue("*Distribution of significant interactions using {sz/1e3}kb bins*")}
gi %>%
  anchors("first") %>%
  seqnames() %>%
  table() %>%
