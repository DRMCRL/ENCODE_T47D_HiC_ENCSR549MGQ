...
plot_grid(
  plot_grid(ngsReports:::.emptyPlot(""), c, ngsReports:::.emptyPlot(""), rel_widths = c(0.03, 0.87, 0.1), nrow = 1),
...
```{r plotIntDist100,fig.height=6, fig.width = 9, fig.cap = glue("*Relationship between counts and distance using {as.numeric(sz)/1e3}kb bins*")}
a <- mcols(gi) %>% 
  as.data.frame %>% 
  ggplot(aes(distance/1e3, counts)) + 
  geom_point(alpha = 0.4) + 
