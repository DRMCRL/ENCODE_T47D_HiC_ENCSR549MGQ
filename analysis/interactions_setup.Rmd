...
By default, any bin pairs with no observed interactions were excluded fro the Max-HiC output.
...
```{r plotInt20, fig.height=6, fig.width = 9, fig.cap = glue("*Distribution of significant interactions using {sz/1e3}kb bins*")}
gi %>%
  anchors("first") %>%
  seqnames() %>%
  table() %>%
...
```{r plotIn40, fig.height=6, fig.width = 9, fig.cap = glue("*Distribution of significant interactions using {sz/1e3}kb bins*")}
gi %>%
  anchors("first") %>%
  seqnames() %>%
  table() %>%
